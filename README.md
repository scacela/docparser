...
3. Run the statement that creates a materialized view from the JSON Collection `KVEXTRACTIONDATA` by referring to [sql.txt](./SQL/docparser.sql).
4. Run the select statement from [sql.txt](.SQL/docparser.sql) to see the contents of the materialized view.
